...
sheet_name <- "ITT_Rb"
group_order <- c("PBS", "R. bicirculan")
group_labels <- c("PBS" = "PBS", "R. bicirculan" = "R.b.")
colors <- c("PBS" = "#2E86C1", "R. bicirculan" = "#D95F02")
fills <- c("PBS" = "#DCEAF7", "R. bicirculan" = "#F6D6C2")
...
ggsave(file.path(results_dir, "figure5_panelE.pdf"), combined, width = 8.0, height = 5.2, device = cairo_pdf)

message("\nPanel E time course statistics")
print(time_stats %>% select(Time, p_value, label))
message("\nPanel E AUC statistics")
print(tibble::tibble(comparison = "PBS vs R.b.", p_value = auc_p, label = p_to_stars(auc_p)))
message("\nDone. PDF file was written to: ", file.path(results_dir, "figure5_panelE.pdf"))